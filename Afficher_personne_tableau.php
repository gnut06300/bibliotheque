...
<html lang="en">
  ...
  <body>
    ...
    while ($donnees = $result->fetch_assoc())
        {
        ?>
            <article class="col-md-4 col-lg-4 col-12 col-sm-12 item-pers"> 
                    <!--mt-1 mb-1 ml-1 mr-1 -->
                    <h2><?= utf8_encode($donnees['nom']).' '.utf8_encode($donnees['prenom']).'</td>' ;?></h2>
                    <img src="img/profil_<?php echo $donnees['numpers'] ?>.jpg" alt="<?= utf8_encode($donnees['nom']) ;?>">
                    <p class="idpers">Id membre : <?= $donnees['numlivre'] ?></p>
                    <p class="ville">Genre : <?= $donnees['ville'] ?></p>
            </article>
        <?php
        /*echo "<tr";
        if ($i%2 == 1) {
